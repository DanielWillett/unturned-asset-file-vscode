...
public class SpecPropertyConverter : JsonConverter<SpecProperty?>
{
    ...
    private static readonly JsonEncodedText MinimumCountProperty = JsonEncodedText.Encode("MinimumCount");
    private static readonly JsonEncodedText MaximumCountProperty = JsonEncodedText.Encode("MaximumCount");
    ...
    [
        ...
        MinimumCountProperty,               // 34
        MaximumCountProperty,               // 35
    ];
    ...
    public static SpecProperty? ReadProperty(IAssetSpecDatabase? database, ref Utf8JsonReader reader, JsonSerializerOptions? options)
    {
        ...
        while (reader.Read() && reader.TokenType != JsonTokenType.EndObject)
        {
            ...
            switch (propType)
            {
                ...
                case 34: // MinimumCount
                case 35: // MaximumCount
                    ISpecDynamicValue? ctRestriction = null;
                    switch (reader.TokenType)
                    {
                        case JsonTokenType.Number:
                            if (!reader.TryGetInt32(out int v) || v < 0)
                                throw new JsonException($"Invalid number reading SpecProperty.\"{Properties[propType].ToString()}\".");
                            ctRestriction = SpecDynamicValue.Int32(v);
                            break;
                    ...
                    if (propType == 34)
                        property.MinimumCount = ctRestriction;
                    else
                        property.MaximumCount = ctRestriction;
                    break;
            }
        }
    ...
    public static void WriteProperty(Utf8JsonWriter writer, SpecProperty? property, JsonSerializerOptions options)
    {
        ...
        if (property.MinimumCount != null)
        {
            writer.WritePropertyName(MinimumCountProperty);
            property.MinimumCount.WriteToJsonWriter(writer, options);
        }
        if (property.MaximumCount != null)
        {
            writer.WritePropertyName(MaximumCountProperty);
            property.MaximumCount.WriteToJsonWriter(writer, options);
        }
        ...
        if (property.Variable != null
            && (
                property.Owner is not IPropertiesSpecType { AutoGeneratedKeys: true }
                || property.Variable is not SpecDynamicConcreteValue<string> str
                || !string.Equals(str.Value, property.Key, StringComparison.Ordinal)
                )
            )
        {
