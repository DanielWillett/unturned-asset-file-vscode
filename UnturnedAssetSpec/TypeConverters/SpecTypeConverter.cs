...
public sealed class SpecTypeConverter : JsonConverter<ISpecType?>
{
    ...
    [
        ...
        VersionProperty,                // 9
        IsFlagsProperty,                // 10
        AutoGeneratedKeysProperty,      // 11
        OverridablePropertiesProperty,  // 12
        DefaultValueProperty            // 13
    ];
    ...
    public static ISpecType? ReadType(IAssetSpecDatabase? database, ref Utf8JsonReader reader, JsonSerializerOptions? options)
    {
        ...
        bool autoGenedKeys = false, overridableProperties = false;
        ...
        while (reader.Read() && reader.TokenType != JsonTokenType.EndObject)
        {
            ...
            switch (propType)
            {
                ...
                case 11: // AutoGeneratedKeys
                    if (reader.TokenType is not JsonTokenType.True and not JsonTokenType.False and not JsonTokenType.Null)
                        ThrowUnexpectedToken(reader.TokenType, propType);

                    autoGenedKeys = reader.TokenType == JsonTokenType.True;
                    break;

                case 12: // OverridableProperties
                    if (reader.TokenType is not JsonTokenType.True and not JsonTokenType.False and not JsonTokenType.Null)
                        ThrowUnexpectedToken(reader.TokenType, propType);

                    overridableProperties = reader.TokenType == JsonTokenType.True;
                    break;

                case 13: // DefaultValue
                    if (reader.TokenType is not JsonTokenType.StartObject and not JsonTokenType.Null)
                        ThrowUnexpectedToken(reader.TokenType, propType);
        ...
        if (autoGenedKeys)
        {
            Array.ForEach(properties, x => x.Variable ??= SpecDynamicValue.String(x.Key));
            Array.ForEach(localProperties, x => x.Variable ??= SpecDynamicValue.String(x.Key));
        }
        ...
        CustomSpecType customType = new CustomSpecType
        {
            ...
            Version = version,
            AutoGeneratedKeys = autoGenedKeys,
        ...
        {
            customType.DefaultValue = ReadDefaultValue(ref defaultValueReader, customType, options, null);
        }

        return customType;
    }

    internal static CustomSpecTypeInstance? ReadDefaultValue(ref Utf8JsonReader reader, CustomSpecType type, JsonSerializerOptions? options, IAssetSpecDatabase? database)
    {
        if (database == null &&
            (!type.Properties.All(x => x.Type is { IsSwitch: false, Type: not ISecondPassSpecDynamicValue })
             || !type.LocalizationProperties.All(x => x.Type is { IsSwitch: false, Type: not ISecondPassSpecDynamicValue })
             ))
        {
            type.DeferredDefaultValue = JsonDocument.ParseValue(ref reader);
            return null;
        }

        // TokenType = StartObject
        List<CustomSpecTypeProperty> properties = new List<CustomSpecTypeProperty>(8);
        while (reader.Read() && reader.TokenType != JsonTokenType.EndObject)
        {
            if (reader.TokenType != JsonTokenType.PropertyName)
            {
                throw new JsonException($"Unexpected token reading default value for type \"{type.Type}\".");
            }

            string propertyName = reader.GetString()!;

            if (!reader.Read())
                throw new JsonException($"Failed to read value for property \"{propertyName}\" while reading default value for type \"{type.Type}\".");

            SpecProperty[] primaryProps, secondaryProps;
            if (propertyName.StartsWith("$local$::", StringComparison.Ordinal))
            {
                propertyName = propertyName[9..];
                primaryProps = type.LocalizationProperties;
                secondaryProps = type.Properties;
            }
            else 
            {
                if (propertyName.StartsWith("$prop$::", StringComparison.Ordinal))
                    propertyName = propertyName[8..];
                primaryProps = type.Properties;
                secondaryProps = type.LocalizationProperties;
            }

            int index = Array.FindIndex(primaryProps, p => string.Equals(p.Key, propertyName, StringComparison.OrdinalIgnoreCase));
            SpecProperty? property;
            if (index >= 0)
            {
                property = primaryProps[index];
            }
            else
            {
                index = Array.FindIndex(secondaryProps, p => string.Equals(p.Key, propertyName, StringComparison.OrdinalIgnoreCase));
                property = index >= 0 ? secondaryProps[index] : null;
            }
            
            if (property == null)
            {
                throw new JsonException($"Unknown property \"{propertyName}\" reading default value for type \"{type.Type}\".");
            }

            try
            {
                ISpecDynamicValue value = SpecDynamicValue.Read(ref reader, options, SpecDynamicValueContext.AllowConditionals, property.Type, false);
                properties.Add(new CustomSpecTypeProperty(value, property, propertyName));
            }
            catch (JsonException ex)
            {
                throw new JsonException(
                    $"Error reading property \"{property.Key}\" (\"{propertyName}\") while reading default value for type \"{type.Type}\".",
                    ex
                );
            }
        }

        return new CustomSpecTypeInstance(type, properties);
    }
    ...
    public static void WriteType(Utf8JsonWriter writer, ISpecType? value, JsonSerializerOptions options)
    {
        ...
        switch (value)
        {
            ...
            case CustomSpecType customType:
                ...
                if (customType.AutoGeneratedKeys)
                    writer.WriteBoolean(AutoGeneratedKeysProperty, true);

                if (customType.OverridableProperties)
