...
internal sealed class UInt8TypeConverter : ITypeConverter<byte>
{
    ...
    public bool TryConvertTo<TTo>(Optional<byte> obj, out Optional<TTo> result) where TTo : IEquatable<TTo>
    {
        ...
        if (typeof(TTo).IsPrimitive)
        {
            ...
            {
                result = SpecDynamicExpressionTreeValueHelpers.As<short, TTo>(value);
                return true;
            }
            ...
            {
                result = SpecDynamicExpressionTreeValueHelpers.As<ushort, TTo>(value);
                return true;
            }
        ...
        if (VectorConversionHelper.TryConvertToVector<byte, TTo>(value, out TTo? parsedVector))
        {
            result = parsedVector;
            return true;
        }
    ...
    public void WriteJson(Utf8JsonWriter writer, byte value, ref TypeConverterFormatArgs args)
    {
