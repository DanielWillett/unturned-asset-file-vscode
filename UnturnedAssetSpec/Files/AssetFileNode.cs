...
public abstract class AssetFileNode
{
    ...
    public required FileRange Range { get; init; }
...
public class AssetFileDictionaryValueNode : AssetFileBlockValueNode
{
    ...
    public bool TryGetValue(SpecProperty property, out AssetFileValueNode? value)
    {
        if (TryGetValue(property, out AssetFileKeyValuePairNode? kvp))
        {
            value = kvp.Value;
            return true;
        }
    ...
    public bool TryGetValue(string key, out AssetFileValueNode? value)
    {
        if (TryGetValue(key, out AssetFileKeyValuePairNode? kvp))
        {
            value = kvp.Value;
            return true;
        }
    ...
    public bool TryGetValue(SpecProperty property, out AssetFileKeyValuePairNode value)
    {
        if (TryGetValue(property.Key, out value))
            return true;

        for (int i = 0; i < Pairs.Count; ++i)
        {
            AssetFileKeyValuePairNode pair = Pairs[i];
            string key = pair.Key.Value;
            bool found = false;
            for (int a = 0; a < property.Aliases.Length; ++a)
            {
                if (!key.Equals(property.Aliases[a], StringComparison.OrdinalIgnoreCase))
                    continue;
    ...
    public bool TryGetValue(string key, out AssetFileKeyValuePairNode value)
    {
        for (int i = 0; i < Pairs.Count; ++i)
        {
            AssetFileKeyValuePairNode pair = Pairs[i];
